    ...
    <div className="container mx-auto px-4 relative my-6">
      ...
      <div ref={scrollElement} className="flex items-center gap-4 md:gap-6 overflow-x-auto scrollbar-none transition-all">
        {loading ? (
          ...
          data.map((product) => (
            <Link to={`/productDetails/${product?._id}`} key={product._id} className="w-full min-w-[270px] md:min-w-[310px] max-w-[270px] md:max-w-[310px] h-40 bg-white rounded shadow flex">
              {/* Left Section */}
              <div className="bg-slate-100 p-5 h-full min-w-[120px] md:min-w-[145px]">
